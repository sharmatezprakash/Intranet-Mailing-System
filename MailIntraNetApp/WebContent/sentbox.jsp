...
<%
	if ((session.getAttribute("mailsFrom") == null) || !(session.getAttribute("mailsFrom").equals("sentbox"))) {
		redirect = true;
	} else {
		redirect = false;
		...
		if (obj instanceof List<?>) {
			...
			sentboxSize = (int) ((session.getAttribute("sentboxSize") != null)
					...
					: -1);
			end = ((start + 10) > sentboxSize) ? sentboxSize : (start + 10);
...
<script src="js/sentbox.js"></script>
<script>
	var startMail;
	var end;
	var arrayOfMailIds = [];
...
<body>
	<form action="GetMails" id='getMails'>
		<input type='hidden' name='isSender' value='true' id='GetMails_category'>
 ...
 %>
					...
					<ul class="inbox-nav inbox-divider">
						...
						<li class="active"><a href="sentbox.jsp"><i class="fa fa-envelope-o"></i>
				...
				<div id='inbox'>
					...
					<div class="inbox-body">
						<div class="mail-option">
							<div class="chk-all">
								<input id='all-chkbox' type="checkbox" onclick='javascript:allChkboxClicked();'
							...
							<div class="btn-group" id='refresh-btn'
								onclick='javascript:loadSentbox();'>
							...
							<ul class="unstyled inbox-pagination">
								<li><span> <%
 	if (mails != null) {
 		out.println((start + 1) + " - " + end + " of " + sentboxSize);
 %>
								</span></li>
								<li><a class="np-btn"
									href="javascript:loadSentbox(<%=start - 10%>)"><i
										class="fa fa-angle-left  pagination-left"></i></a></li>
								<li><a class="np-btn" href="javascript:loadSentbox(<%=end%>)"><i
						...
						<table class="table table-inbox table-hover">
							<tbody>
								...
								<tr class="unread">
									...
									<td class="inbox-small-cells"><i <%=mails.get(i).isStarred() ? "class='fa fa-star showStarred'" : "class='fa fa-star'"%>
										id='starred-<%=mailId%>'
										onclick='javascript:starredClick(<%=mailId%>)'></i></td>
									<td class="view-message  dont-show"><div
											id='viewMsg-<%=mailId%>'
				...
				<form action='ViewMail.jsp' id='viewMailForm'>
					<input type='hidden' name='isSender' value='true' id='isSender'>
				...
				<form action='MarkStarred' id='markStarredForm' method='Post'>
					...
					<input type='hidden' name='isMarked' value='' id='markStarredForm_isMarked' /> 
					<input type='hidden' name='isSender' value='' id='markStarredForm_isSender'>
					<input type='hidden' name='start' value='' id='markStarredForm_start'>
				</form>
				<form id='DeleteMail' action='DeleteMail' method='post'>
					<input type='hidden' name='isSender' id='DeleteMail_isSender' value='true'>
